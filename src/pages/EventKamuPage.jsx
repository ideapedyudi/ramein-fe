  ...
  return (
    <div className="min-h-screen bg-[var(--color-page)]">
      <div className="border-b border-black/5 bg-white">
        <div className="mx-auto flex max-w-[1280px] flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          <Link to="/home" className="flex items-center gap-3">
            <span className="text-sm text-gray-600 hover:text-brand-600">← Kembali</span>
            <span className="text-lg font-bold text-gray-900 sm:text-xl">Event Kamu</span>
          </Link>
          <Link
            to="/buat-event"
          ...
          {events.map((event) => (
            ...
            >
              <div className="flex flex-col gap-6 p-4 sm:flex-row sm:p-5">
                <div
                  className={`relative h-44 w-full shrink-0 overflow-hidden rounded-xl bg-gradient-to-br sm:h-44 sm:w-60 ${event.bannerHue ?? 'from-brand-400 to-brand-600'}`}
                >
                  {event.imageUrl && (
                    <img
                      src={event.imageUrl}
